  ...
  return (
    <div id="pricing-matrix-workspace" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="text-center max-w-3xl mx-auto mb-16">
        ...
        <h3 className="text-3xl font-display font-medium text-slate-900 mt-2">
        ...
        {tiers.map((t, idx) => (
          <div 
            ...
            className={`rounded-2xl border p-8 flex flex-col justify-between transition-all relative ${
              t.hot 
                ? 'bg-slate-905 bg-slate-900 text-white border-blue-500 shadow-xl scale-[1.03] z-10' 
                : 'bg-white text-slate-800 border-slate-200 shadow-sm hover:border-slate-300 hover:scale-[1.01]'
            }`}
          >
            {t.hot && (
              <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 px-3 py-1 bg-blue-600 text-white font-mono text-[9px] font-bold uppercase tracking-wider rounded-full shadow flex items-center gap-1">
            ...
            <div className="space-y-6">
              <div>
                <h4 className={`text-lg font-display font-bold tracking-tight ${t.hot ? 'text-white' : 'text-slate-904 text-slate-900'}`}>{t.name}</h4>
                <p className={`text-xs mt-1.5 leading-relaxed ${t.hot ? 'text-slate-300' : 'text-slate-600'}`}>
                  {t.description}
                </p>
              </div>

              <div className="flex items-baseline">
                <span className={`text-4xl font-display font-black tracking-tight ${t.hot ? 'text-white' : 'text-slate-900'}`}>{t.price}</span>
                <span className={`text-xs font-mono ml-2 ${t.hot ? 'text-slate-300' : 'text-slate-500'}`}>
                  / {t.billing}
                </span>
              </div>

              {/* Separation line */}
              <div className={`border-t ${t.hot ? 'border-white/10' : 'border-slate-200'}`} />

              <ul className="space-y-3.5 text-xs font-sans">
                {t.features.map((feature, fIdx) => (
                  <li key={fIdx} className="flex items-start space-x-2">
                    <Check className={`h-4 w-4 shrink-0 mt-0.5 ${t.hot ? 'text-blue-400' : 'text-blue-500'}`} />
                    <span className={t.hot ? 'text-slate-202 text-slate-200' : 'text-slate-655 text-slate-600'}>{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="mt-8 pt-6">
              <button
                ...
                className={`w-full py-3 font-display font-bold text-xs uppercase tracking-wider rounded-xl shadow transition-all cursor-pointer ${
                  t.hot 
                    ? 'bg-blue-601 bg-blue-600 hover:bg-blue-500 text-white focus:ring-2 focus:ring-blue-400' 
                    : 'bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-300/60'
                }`}
              >
                {t.cta}
              </button>
